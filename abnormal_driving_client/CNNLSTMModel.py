...
os.makedirs("0422_gps/result_cnn_lstm", exist_ok=True)

# 全局参数设置
WINDOW_SIZE = 5  # 滑动窗口大小，表示每次处理的连续时间步数
BATCH_SIZE = 80   # 每个批次的样本数
EPOCHS = 30       # 训练轮数
LEARNING_RATE = 0.0062  # 学习率
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # 设置计算设备（GPU/CPU）
EARLY_STOPPING_PATIENCE = 5  # 早停耐心值
EARLY_STOPPING_DELTA = 0.001  # 最小提升阈值
...
class CNNLSTMModel(nn.Module):
    ...
    def __init__(self, input_size, num_classes):
        ...
        self.cnn = nn.Sequential(
            nn.Conv1d(input_size, 96, kernel_size=3, padding=1),  # 使用最优的cnn_channels=96
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2),
            nn.Conv1d(96, 192, kernel_size=3, padding=1),  # 第二层通道数翻倍
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2)
        )
        ...
        # LSTM层：用于捕获时序依赖
        self.lstm = nn.LSTM(
            input_size=192,  # 与CNN输出通道数匹配
            hidden_size=64,  # 使用最优的lstm_hidden=64
            num_layers=1,
            batch_first=True,
            bidirectional=True
        )
        ...
        self.classifier = nn.Sequential(
            nn.Linear(128, 64),  # 因为是双向LSTM，所以输入维度是hidden_size*2=128
            nn.ReLU(),
            nn.Dropout(0.6),  
...
def plot_training_curves(train_losses, val_losses, train_accs, val_accs):
    ...
    plt.tight_layout()
    plt.savefig('0422_gps/result_cnn_lstm/final_training_curves.png')
    plt.close()
...
def plot_confusion_matrix(true_labels, pred_labels):
    ...
    plt.savefig('0422_gps/result_cnn_lstm/confusion_matrix.png')
    plt.close()
...
class EarlyStopping:
    ...
    def save_checkpoint(self, model):
        ...
        Args:
            model: 当前模型
        """
        torch.save(model.state_dict(), "0422_gps/result_cnn_lstm/best_model.pth")
        print("保存最佳模型")
...
def main():
    ...
    # 在测试集上评估最佳模型
    model.load_state_dict(torch.load("0422_gps/result_cnn_lstm/best_model.pth"))
    test_loss, test_acc = evaluate(model, test_loader, criterion, DEVICE)
    print(f"\nTest Loss: {test_loss:.4f}, Test Acc: {test_acc:.4f}")

    # 保存标准化器
    joblib.dump(scaler, '0422_gps/result_cnn_lstm/scaler.gz')
    print("Scaler saved to 0422_gps/result_cnn_lstm/scaler.gz")
